...
public class RegisteredUser {
	...
	public double spendChips(double cantidad) {
		if(this.chips - cantidad >= 0) {
			this.chips = this.chips - cantidad;
			return this.chips;
		} else {
			throw new RuntimeException("Usuario  gasta mas de lo que tiene");
		}
	}
	...
	public void deleteIngredient(Ingredients ingrediente1, Recipe receta) {
		...
		if(this.listaDeRecetas.contains(receta)) {
			if(receta.getListaIngredientes().contains(ingrediente1)) {
				...
				throw new RuntimeException("You can not delete an ingredient that is not in the recipe");
			}
		}
		else {
			throw new RuntimeException("You can not delete an ingredient from a recipe that  you have not done");
		}
	...
	public void reportRecipe(Recipe recipe, Report report1) {	
		if(recipe.getListaReports() == null) {
			List<Report> newList = new ArrayList<>();
			newList.add(report1);
			recipe.setListaReports(newList);
		} else {
			int i, cnt = 0;
			for (i = 0; i < recipe.getListaReports().size(); i++) {
				if (recipe.getListaReports().get(i).getReporting() == report1.reporting)
					cnt++;
			}		
			if (cnt > 0) {
				throw new RuntimeException("The recipe has already been reported");
			} else {
				recipe.getListaReports().add(report1);
			}
		}
	}
}